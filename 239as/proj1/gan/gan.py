...
def sample_noise(batch_size, dim, seed=None):
    ...
    if seed:
        return (torch.rand(batch_size, dim, generator=seed) - 0.5)*2
    else:
...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    real_labels = torch.ones(logits_real.size(dim=0)).type(dtype)
    real_loss = bce_loss(logits_real, real_labels)
    fake_labels = torch.zeros(logits_fake.size(dim=0)).type(dtype)
...
def generator_loss(logits_fake):
    ...
    fake_labels = torch.zeros(logits_fake.size(dim=0)).type(dtype)
    loss = bce_loss(-1*logits_fake, fake_labels)
...
def build_dc_classifier(batch_size):
    ...
    return nn.Sequential(
            ...
            Flatten(),
            nn.Linear(in_features=4*4*64, out_features=4*4*64),  # Output size 4*4*64
            nn.LeakyReLU(negative_slope=0.01),
            nn.Linear(in_features=4*4*64, out_features=1)  # Output size 1
    )
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    return nn.Sequential(
            ...
            Unflatten(),  # Define Unflatten as a custom module
